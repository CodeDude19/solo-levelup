...
/**
 * Top navigation bar with gooey indicator animation (indicator below tabs)
 */
const TopNav = forwardRef(({ tabs, activeTab, onTabChange, swipeProgress, tabOrder }, ref) => {
  ...
  return (
    <nav ref={ref} className="bg-cyber-dark border-b border-cyber-cyan/20 px-2 py-2 safe-area-top relative">
      ...
      {/* Gooey Indicator - positioned below nav */}
      <div
      ...
      >
        ...
        >
          {/* Main gooey blob */}
          <div
            className="absolute top-0 left-1/2 h-1 bg-cyber-cyan rounded-full"
            style={{
          ...
          {Math.abs(swipeProgress) > 0.1 && (
            <div
              className="absolute top-0 left-1/2 h-1 bg-cyber-cyan/40 rounded-full"
              style={{
